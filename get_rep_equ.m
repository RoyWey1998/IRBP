function [rep_equ] = get_rep_equ(norm, strategy, mu_e, mu_G, mu_B)
% Get reputation equilibrium under public assessment (covering JTB 2004)
% Input:
...
reputation_dynamics = get_rep_dym(norm, strategy, mu_e, mu_G, mu_B);
...
for i = 1:length(possible_equ_vals)
    ...
    if candidate >= 0 && candidate <= 1
        if isempty(rep_equ)
            ...
            if all(abs(rep_equ - candidate) > 1e-4)
                rep_equ = [rep_equ; candidate];
            end
        end
    end
